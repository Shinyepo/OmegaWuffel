...
namespace OWuffel.Modules.Commands.Events
{
    public class Events : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task LogsAsync(string key = "", string value = "")
        {
            try
            {
                var logsConfig = await EventsService.GetLogsConfigsAsync(Context.Guild.Id);
                if (logsConfig == null)
                {
                    await ReplyAsync("Something went terribly wrong :(");
                    return;
                }
                var setting = logsConfig
                        .GetType()
                        .GetProperties()
                        ...
                        {
                            Name = pi.Name,
                            Value = pi.GetGetMethod().Invoke(logsConfig, null)
                        });
                if (key == "")
                {
                ...
                else
                {
                    ...
                    if (value != "")
                    {
                        ...
                        await DbUtilities.SetSettingsValueAsync(Context.Guild, found.Name, chnl.Id);
                        await ReplyAsync($"{found.Name.Replace("Log", "")} related logs will be sent to {chnl.Mention}.");
                    }
                    else
                    {
                        await DbUtilities.SetSettingsValueAsync(Context.Guild, found.Name, 0);
